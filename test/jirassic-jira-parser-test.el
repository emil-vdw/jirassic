...
(ert-deftest jirassic-parser-test-parse-issue ()
  (let ((issue (jirassic--parse-issue
    ...
    (should (null (jira-issue-attachments issue)))))

(ert-deftest jirassic-parser-test-parse-media-single ()
  (let* ((node (jirassic-parse-adf-node
                '((type . "mediaSingle")
                  (attrs (width . 700) (widthType . "pixel")
                         (layout . "align-start"))
                  (content . [((type . "media")
                               (attrs (type . "file")
                                      ...
                                      (alt . "screenshot.png")
                                      (collection . "")))]))))
         (media (car (adf-media-single-content node))))
    (should (cl-typep node 'adf-media-single))
    (should (cl-typep media 'adf-media))
    (should (string= (adf-media-id media) "b80d623d-f885-408b-b150-2c8843a3ac1b"))
    (should (eq (adf-media-media-type media) 'file))
    (should (string= (adf-media-alt media) "screenshot.png"))
    (should (string= (adf-media-collection media) ""))))

(ert-deftest jirassic-parser-test-parse-media-group ()
  (let* ((node (jirassic-parse-adf-node
                '((type . "mediaGroup")
                  (content . [((type . "media")
                               (attrs (type . "file")
                              ...
                              ((type . "media")
                               (attrs (type . "file")
                                      ...
                                      (alt . "b.png")
                                      (collection . "")))]))))
         (media (adf-media-group-content node)))
    (should (cl-typep node 'adf-media-group))
    (should (= (length media) 2))
    (should (string= (adf-media-alt (car media)) "a.png"))
    (should (string= (adf-media-alt (cadr media)) "b.png"))))

(ert-deftest jirassic-parser-test-parse-media-without-alt ()
  "`attrs.alt' is officially optional per the ADF spec."
  (let ((media (jirassic-parse-adf-node
                '((type . "media")
                  (attrs (type . "file")
                         (id . "6e7c7f2c-dd7a-499c-bceb-6f32bfbf30b5")
                         (collection . "ae730abd"))))))
    (should (cl-typep media 'adf-media))
    (should (null (adf-media-alt media)))
    (should (eq (adf-media-media-type media) 'file))))

(ert-deftest jirassic-parser-test-parse-issue-attachments ()
  (let* ((issue (jirassic--parse-issue
                 '((id . "10001")
                   (key . "PROJ-1")
                   (fields
                    ...
                    (attachment
                     . [((id . "10042")
                        ...
                        ((id . "10043")
                         ...
                         (mimeType . "text/plain")
                         (size . 1024)
                         (content . "https://acme.atlassian.net/rest/api/3/attachment/content/10043"))])))))
         (attachments (jira-issue-attachments issue)))
    (should (= (length attachments) 2))
    (let ((first (car attachments)))
      (should (cl-typep first 'jira-attachment))
      (should (string= (jira-attachment-id first) "10042"))
      (should (string= (jira-attachment-filename first) "screenshot.png"))
      (should (string= (jira-attachment-mime-type first) "image/png"))
      (should (= (jira-attachment-size first) 84512))
      (should (string= (jira-attachment-content-url first)
                       "https://acme.atlassian.net/rest/api/3/attachment/content/10042"))
      (let ((author (jira-attachment-author first)))
        (should (cl-typep author 'jira-user))
        (should (string= (jira-user-display-name author) "Jane Doe"))))
    (let ((second (cadr attachments)))
      (should (string= (jira-attachment-filename second) "log.txt"))
      ;; Author is optional.
      (should (null (jira-attachment-author second))))))
